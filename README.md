...
- txt转epub时可指定元数据(书名, 作者, 协作, 简介, 语言, 发行日期, 出版商等), 封面图, 插图, css样式, 章节识别正则
- 内置简单的css样式提升生成的epub电子书阅读体验(正文, 章节标题, 人物对话, 时间, 代码块)
...
# 工具内置一套简单的css样式(body:正文, h2:章节标题, p:段落, span:对话, data:书名或专有词, time:时间 pre:代码块), 命令参数指定css样式将忽略内置样式
# 时间高亮由于存在误匹配, 默认不开启, 可指定-v参数开启
# 工具内置多套章节识别正则表达式, 并自动使用最符合的正则表达式切分章节, 命令参数指定章节识别正则表达式将忽略内置表达式
iepub -t <书名> -v -a <作者> -x <协作> -g <语言> -e <描述> -k <标签> -d <发行日> -u <出版商> -b <版权声明> -c <封面图> -z <插图> -s <css样式> -m <图片标签识别正则> -r <章节识别正则> -o <输出文件> xxx.txt
